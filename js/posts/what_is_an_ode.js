...
const a_function = () => {
  ...
  // Define the scales and axes
  const x = d3.scaleLinear().domain([xmin, xmax]).range([0, width]);
  const y = d3.scaleLinear().domain([xmin, xmax]).range([height, 0]);
  ...
  // Generate sine wave data
  const data = d3.range(xmin, xmax, 0.1).map(function (d) {
    return { x: d, y: d };
  });
